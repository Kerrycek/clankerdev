...
export function UserNamespaceList(props: {
  ...
  return (
    <div className="space-y-3" data-testid={`${props.testIdPrefix}.page`}>
      <FilterBar testId={`${props.testIdPrefix}.filters`}>
        <div className="min-w-0 flex-1">
          <SmartFilterInput
            ...
            suffix={
              <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-md text-muted hover:bg-surface-2 hover:text-fg" onClick={() => setHelpOpen(true)} aria-label={t("filters.help.open")} title={t("filters.help.open")} data-testid={`${props.testIdPrefix}.smart.help_button`}>
        ...
        <div className="flex items-center gap-2">
          <Button type="button" variant="secondary" size="sm" onClick={() => setAdvancedOpen(true)} testId={`${props.testIdPrefix}.filters.advanced`}>
      ...
      {q || size !== undefined || (props.showAdminFields && !props.fixedUserId && (userId !== undefined || blockCount !== undefined)) || smartErrors.length > 0 ? (
        <div className="flex flex-wrap gap-2" data-testid={`${props.testIdPrefix}.filters.chips`}>
          ...
          {smartErrors.map((err, idx) => (
            <FilterChip key={`${err}-${idx}`} label={err} tone="danger" onRemove={() => setSmartErrors((cur) => cur.filter((_, i) => i !== idx))} />
          ))}
        </div>
      ) : null}
      {qList.isLoading ? (
        ...
        <>
          <EmptyState testId={`${props.testIdPrefix}.empty`} title={t("userns.namespace.empty.title")} body={filtersActive ? t("userns.namespace.empty.filtered") : t("userns.namespace.empty.body")} actionLabel={filtersActive ? t("common.clear_filters") : undefined} onAction={filtersActive ? clearFilters : undefined} />
          <Card className="mt-4">
            <KeysetPagination page={pagination.page} pageCount={pagination.stack.length} canPrev={pagination.canPrev} canNext={canNext} onPrev={pagination.goPrev} onNext={() => pagination.goNext(pageCursor)} onGoToPage={pagination.goToPage} limit={pagination.limit} allowedLimits={pagination.allowedLimits} onLimitChange={pagination.setLimit} testId={`${props.testIdPrefix}.pagination`} />
          </Card>
        </>
      ) : (
        <TableCard testId={`${props.testIdPrefix}.table`} minWidth="lg" footer={<KeysetPagination page={pagination.page} pageCount={pagination.stack.length} canPrev={pagination.canPrev} canNext={canNext} onPrev={pagination.goPrev} onNext={() => pagination.goNext(pageCursor)} onGoToPage={pagination.goToPage} limit={pagination.limit} allowedLimits={pagination.allowedLimits} onLimitChange={pagination.setLimit} testId={`${props.testIdPrefix}.pagination`} />}>
          <thead>
            ...
            {rows.map((r) => {
              ...
              return (
                <TableRowLink key={r.id} to={to} testId={`${props.testIdPrefix}.row.${r.id}`} className="border-b border-border">
                  ...
                  {props.showAdminFields ? <td className="px-4 py-2 text-sm">{userLabel((r as LegacyAny).user)}</td> : null}
                  {props.showAdminFields ? <td className="px-4 py-2 text-sm tabular-nums">{(r as LegacyAny).block_count ?? "—"}</td> : null}
      ...
      <Drawer open={advancedOpen} onClose={() => setAdvancedOpen(false)} title={t("filters.advanced.title")} width="lg" testId={`${props.testIdPrefix}.filters.advanced.drawer`}>
        <div className="space-y-4">
          <div>
            <div className="text-xs text-muted">{t("common.search")}</div>
            <div className="mt-1">
              <Input testId={`${props.testIdPrefix}.search.advanced`} placeholder={t("userns.namespace.search_placeholder")} value={q} onChange={(e) => setFilters({ q: e.target.value.replace(/^#/, ""), size, userId, blockCount })} />
            </div>
          </div>
          <div>
      ...
      <SmartInputHelp open={helpOpen} onClose={() => setHelpOpen(false)} title={t("filters.help.title")} intro={t("userns.namespace.smart.help.intro")} examples={[{ example: "?", description: t("userns.namespace.smart.help.examples.help") }, { example: "123", description: t("userns.namespace.smart.help.examples.open_id") }, { example: "size:65536", description: t("userns.namespace.smart.help.examples.size") }, ...(props.showAdminFields && !props.fixedUserId ? [{ example: "user:42", description: t("userns.namespace.smart.help.examples.user") }] : [])]} topKeys={[{ key: "id", description: t("userns.namespace.smart.help.keys.id"), example: "id:123" }, { key: "size", description: t("userns.namespace.smart.help.keys.size"), example: "size:65536" }, ...(props.showAdminFields && !props.fixedUserId ? [{ key: "user", description: t("userns.namespace.smart.help.keys.user"), example: "user:42" }] : []), ...(props.showAdminFields && !props.fixedUserId ? [{ key: "blocks", description: t("userns.namespace.smart.help.keys.blocks"), example: "blocks:3" }] : [])]} inference={[t("userns.namespace.smart.help.inference.enter_applies"), t("userns.namespace.smart.help.inference.number_opens"), t("userns.namespace.smart.help.inference.key_value")]} onInsertKey={insertSmartKey} testId={`${props.testIdPrefix}.smart.help`} />
    </div>
  );
}
