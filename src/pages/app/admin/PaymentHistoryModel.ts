...
export function paymentHistoryDateBoundary(value: string, endOfDay = false): string | undefined {
  const normalized = value.trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(normalized)) return undefined;
  const timestamp = `${normalized}T${endOfDay ? '23:59:59.999' : '00:00:00.000'}Z`;
