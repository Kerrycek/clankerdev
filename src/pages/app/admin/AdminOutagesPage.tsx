  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2">
        <div><FormField label={t('admin.outages.field.begins_at')}><Input testId="admin.outages.form.begins_at" type="datetime-local" value={form.beginsAt} onChange={(e) => setForm((p) => ({ ...p, beginsAt: e.target.value }))} /></FormField><ErrorText message={errors.beginsAt} /></div>
        <FormField label={t('admin.outages.field.finished_at')}><Input testId="admin.outages.form.finished_at" type="datetime-local" value={form.finishedAt} onChange={(e) => setForm((p) => ({ ...p, finishedAt: e.target.value }))} /></FormField>
        <div><FormField label={t('admin.outages.field.duration')}><Input testId="admin.outages.form.duration" inputMode="numeric" value={form.duration} onChange={(e) => setForm((p) => ({ ...p, duration: e.target.value }))} /></FormField><ErrorText message={errors.duration} /></div>
        <div><FormField label={t('admin.outages.field.impact')}><Select testId="admin.outages.form.impact" value={form.impact} onChange={(e) => setForm((p) => ({ ...p, impact: e.target.value }))} options={IMPACTS.map((v) => ({ value: v, label: t(`outage.impact.${v}` as LegacyAny) }))} /></FormField><ErrorText message={errors.impact} /></div>
        {!props.updateMode ? (
          <div><FormField label={t('admin.outages.field.type')}><Select testId="admin.outages.form.type" value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value }))} options={OUTAGE_TYPES.map((v) => ({ value: v, label: t(`outage.type.${v}` as LegacyAny) }))} /></FormField><ErrorText message={errors.type} /></div>
        ) : null}
        {props.includeState ? (
          <FormField label={t('admin.outages.field.state')}><Select testId="admin.outages.form.state" value={form.state} onChange={(e) => setForm((p) => ({ ...p, state: e.target.value }))} options={STATES.map((v) => ({ value: v, label: t(`admin.outages.state.${v}`) }))} /></FormField>
        ) : null}
      </div>
      {!props.updateMode ? (
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div><FormField label={t('admin.outages.field.en_summary')}><Input testId="admin.outages.form.en_summary" value={form.enSummary} onChange={(e) => setForm((p) => ({ ...p, enSummary: e.target.value }))} /></FormField><ErrorText message={errors.enSummary} /></div>
        <div><FormField label={t('admin.outages.field.cs_summary')}><Input testId="admin.outages.form.cs_summary" value={form.csSummary} onChange={(e) => setForm((p) => ({ ...p, csSummary: e.target.value }))} /></FormField><ErrorText message={errors.csSummary} /></div>
...
function SystemsEditor(props: {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2">
        <div>
          <FormField label={t('admin.outages.systems.environments')}><Select testId="admin.outages.systems.environments.select" value="" onChange={(e) => {
            const id = parsePositiveInt(e.target.value);
            if (id) props.setForm((p) => ({ ...p, environments: appendIdList(p.environments, id) }));
          }} options={envOptions} /></FormField>
          <div className="mt-1 text-xs text-muted">{props.form.environments || t('admin.outages.systems.none_selected')}</div>
        </div>
        <div>
          <FormField label={t('admin.outages.systems.locations')}><Select testId="admin.outages.systems.locations.select" value="" onChange={(e) => {
            const id = parsePositiveInt(e.target.value);
            if (id) props.setForm((p) => ({ ...p, locations: appendIdList(p.locations, id) }));
          }} options={locationOptions} /></FormField>
        ...
        <div>
          <FormField label={t('admin.outages.systems.vpsadmin')}><Input testId="admin.outages.systems.vpsadmin" value={props.form.vpsadmin} onChange={(e) => props.setForm((p) => ({ ...p, vpsadmin: e.target.value }))} /></FormField>
          <div className="mt-1 text-xs text-muted">{t('admin.outages.systems.vpsadmin_help')}</div>
        </div>
        <div>
          <FormField label={t('admin.outages.systems.additional')}><Input testId="admin.outages.systems.additional" value={props.form.additional} onChange={(e) => props.setForm((p) => ({ ...p, additional: e.target.value }))} /></FormField>
      ...
      <Card>
        <CardBody>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <FormField label={t('admin.outages.field.state')}><Select value={state} onChange={(e) => setState(e.target.value)} options={[{ value: '', label: t('common.all') }, ...STATES.map((v) => ({ value: v, label: t(`admin.outages.state.${v}`) }))]} /></FormField>
            <FormField label={t('admin.outages.field.type')}><Select value={type} onChange={(e) => setType(e.target.value)} options={[{ value: '', label: t('common.all') }, ...OUTAGE_TYPES.map((v) => ({ value: v, label: t(`outage.type.${v}` as LegacyAny) }))]} /></FormField>
            ...
            {rows.map((o) => {
              ...
              return (
                <tr key={o.id}>
                  ...
                  <td className="px-3 py-2 text-sm">{String((o as LegacyAny).en_summary || (o as LegacyAny).cs_summary || t('public.outage.fallback_title', { id: o.id }))}</td>
...
function AdminOutageDetailPage({ outageId }: { outageId: number | undefined }) {
  ...
  return (
    <div className="space-y-6" data-testid="admin.outages.detail.page">
      <PageHeader
        title={String((outage as LegacyAny).en_summary || (outage as LegacyAny).cs_summary || t('public.outage.fallback_title', { id: outage.id }))}
        ...
        <Card>
          ...
          <CardBody>
            <dl className="grid gap-3 md:grid-cols-2">
              ...
              <div><dt className="text-xs font-semibold text-muted">{t('admin.outages.field.duration')}</dt><dd>{outage.duration != null ? formatDurationMinutes(outage.duration as LegacyAny) : '—'}</dd></div>
              ...
              {updatesQ.data.map((u) => {
                ...
                return (
                  <div key={u.id} className="rounded-md border border-border p-3">
                    ...
                    <div className="mt-2 text-sm font-medium">{String((u as LegacyAny).en_summary || (u as LegacyAny).cs_summary || t('public.outage_detail.updates.update_fallback', { id: u.id }))}</div>
