...
export function UsersPage() {
  ...
  const filtersActive = Boolean(qText.trim() || role || level !== undefined || mailerEnabled !== undefined || lockout !== undefined || passwordReset !== undefined || mfa !== undefined);
  const clearFilters = () => {
    setSmart("");
    setSmartErrors([]);
    setSp((prev) => {
  ...
  const createM = useMutation({
    ...
    onSuccess: (res) => {
      const user = res.data as LegacyAny;
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (qText.trim()) {
      chips.push(<FilterChip key="q" label={`q:${qText.trim()}`} tone="neutral" onRemove={() => setTextParam("q", undefined)} testId="admin.users.chip.q" />);
    }
    if (role) {
      chips.push(<FilterChip key="role" label={`role:${role}`} tone="neutral" onRemove={() => setTextParam("role", undefined)} testId="admin.users.chip.role" />);
    }
    if (level !== undefined) {
      chips.push(<FilterChip key="level" label={`level:${level}`} tone="neutral" onRemove={() => setTextParam("level", undefined)} testId="admin.users.chip.level" />);
    }
    if (mailerEnabled !== undefined) {
      chips.push(<FilterChip key="mailer" label={`mailer:${mailerEnabled ? "on" : "off"}`} tone="neutral" onRemove={() => setBoolParamInUrl("mailer", undefined)} testId="admin.users.chip.mailer" />);
    }
    if (lockout !== undefined) {
      chips.push(<FilterChip key="lockout" label={`lockout:${lockout ? "on" : "off"}`} tone={lockout ? "danger" : "neutral"} onRemove={() => setBoolParamInUrl("lockout", undefined)} testId="admin.users.chip.lockout" />);
    }
    if (passwordReset !== undefined) {
      chips.push(<FilterChip key="password_reset" label={`password_reset:${passwordReset ? "on" : "off"}`} tone={passwordReset ? "warn" : "neutral"} onRemove={() => setBoolParamInUrl("password_reset", undefined)} testId="admin.users.chip.password_reset" />);
    }
    if (mfa !== undefined) {
      chips.push(<FilterChip key="mfa" label={`mfa:${mfa ? "on" : "off"}`} tone="neutral" onRemove={() => setBoolParamInUrl("mfa", undefined)} testId="admin.users.chip.mfa" />);
    }
    smartErrors.forEach((e, idx) => {
      chips.push(<FilterChip key={`err.${idx}`} label={e} tone="danger" onRemove={() => setSmartErrors([])} testId={`admin.users.chip.error.${idx}`} />);
    });
    return chips;
  }, [lockout, mailerEnabled, mfa, passwordReset, qText, role, level, setBoolParamInUrl, smartErrors, t]);
  ...
  return (
    <ListShell
      ...
      header={<PageHeader title={t("admin.users.title")} description={t("admin.users.subtitle")} meta={<span className="text-xs text-faint">{t("admin.users.filter_hint")}</span>} testId="admin.users.list.header" />}
      filters={
        <>
          <FilterBar testId="admin.users.list.filters">
            <div className="w-full sm:max-w-xl">
              <SmartFilterInput
                ...
                suffix={
                  <Button variant="ghost" size="sm" className="h-8 w-8 px-0" onClick={() => setHelpOpen(true)} aria-label={t("filters.help.open")} title={t("filters.help.open")}>
            ...
            <Button variant="secondary" size="sm" onClick={() => setAdvancedOpen(true)} aria-label={t("filters.advanced.open")} title={t("filters.advanced.open")}>
          ...
          <SmartInputHelp
            ...
            inference={[t("admin.users.smart_help.inference.enter_search"), t("admin.users.smart_help.inference.numeric_open"), t("admin.users.smart_help.inference.advanced")]}
            onInsertKey={(key) => {
              const prefix = `${key}:`;
              setSmart(prefix);
              setHelpOpen(false);
          ...
          />
          <Drawer open={advancedOpen} onClose={() => setAdvancedOpen(false)} title={t("filters.advanced.title")} width="lg" testId="admin.users.advanced.drawer">
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-2">
                  <Select value={role} onChange={(e) => setTextParam("role", normalizeRole(e.target.value) || undefined)} options={roleOptions} testId="admin.users.advanced.role" />
                </div>
              </div>
              <div>
                <div className="text-sm font-medium">{t("admin.users.advanced.level")}</div>
                <div className="mt-2 max-w-xs">
                  <Input value={level !== undefined ? String(level) : ""} onChange={(e) => setTextParam("level", e.target.value.trim() ? e.target.value : undefined)} placeholder={t("admin.users.advanced.level.placeholder")} testId="admin.users.advanced.level" inputMode="numeric" />
                </div>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-2">
                    <Select value={mailerEnabled === undefined ? "" : mailerEnabled ? "1" : "0"} onChange={(e) => setBoolParamInUrl("mailer", parseBoolParam(e.target.value))} options={enabledOptions} testId="admin.users.advanced.mailer" />
                  </div>
                </div>
                <div>
                  <div className="text-sm font-medium">{t("admin.users.advanced.mfa")}</div>
                  <div className="mt-2">
                    <Select value={mfa === undefined ? "" : mfa ? "1" : "0"} onChange={(e) => setBoolParamInUrl("mfa", parseBoolParam(e.target.value))} options={enabledOptions} testId="admin.users.advanced.mfa" />
                  </div>
                </div>
                <div>
                  <div className="text-sm font-medium">{t("admin.users.advanced.lockout")}</div>
                  <div className="mt-2">
                    <Select value={lockout === undefined ? "" : lockout ? "1" : "0"} onChange={(e) => setBoolParamInUrl("lockout", parseBoolParam(e.target.value))} options={lockoutOptions} testId="admin.users.advanced.lockout" />
                  </div>
                </div>
                <div>
                  <div className="text-sm font-medium">{t("admin.users.advanced.password_reset")}</div>
                  <div className="mt-2">
                    <Select value={passwordReset === undefined ? "" : passwordReset ? "1" : "0"} onChange={(e) => setBoolParamInUrl("password_reset", parseBoolParam(e.target.value))} options={passwordResetOptions} testId="admin.users.advanced.password_reset" />
                  </div>
                </div>
              </div>
              {filtersActive ? (
          ...
          <Modal
            ...
            footer={
              <div className="flex items-center justify-end gap-2">
                ...
                <Button variant="primary" onClick={() => createM.mutate()} loading={createM.isPending} testId="admin.users.create.submit">
          ...
          >
            <div className="space-y-4">
              {createError ? <div className="rounded-md border border-danger-border bg-danger-surface px-3 py-2 text-sm text-danger">{createError}</div> : null}
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <span className="text-sm font-medium">{t("admin.users.create.field.login")}</span>
                  <Input value={createDraft.login} onChange={(e) => setCreateField("login", e.target.value)} autoComplete="off" testId="admin.users.create.login" />
                </label>
                <label className="block">
                  <span className="text-sm font-medium">{t("admin.users.create.field.level")}</span>
                  <Input value={createDraft.level} onChange={(e) => setCreateField("level", e.target.value)} inputMode="numeric" testId="admin.users.create.level" />
                </label>
                <label className="block">
                  <span className="text-sm font-medium">{t("admin.users.create.field.password")}</span>
                  <Input type="password" value={createDraft.password} onChange={(e) => setCreateField("password", e.target.value)} autoComplete="new-password" testId="admin.users.create.password" />
                </label>
                <label className="block">
                  <span className="text-sm font-medium">{t("admin.users.create.field.password2")}</span>
                  <Input type="password" value={createDraft.password2} onChange={(e) => setCreateField("password2", e.target.value)} autoComplete="new-password" testId="admin.users.create.password2" />
                </label>
                <label className="block">
                  <span className="text-sm font-medium">{t("admin.users.create.field.full_name")}</span>
                  <Input value={createDraft.fullName} onChange={(e) => setCreateField("fullName", e.target.value)} testId="admin.users.create.full_name" />
                </label>
                <label className="block">
                  <span className="text-sm font-medium">{t("admin.users.create.field.email")}</span>
                  <Input type="email" value={createDraft.email} onChange={(e) => setCreateField("email", e.target.value)} testId="admin.users.create.email" />
                </label>
                <label className="block">
                  <span className="text-sm font-medium">{t("admin.users.create.field.monthly_payment")}</span>
                  <Input value={createDraft.monthlyPayment} onChange={(e) => setCreateField("monthlyPayment", e.target.value)} inputMode="decimal" testId="admin.users.create.monthly_payment" />
                </label>
                <Checkbox checked={createDraft.mailerEnabled} onChange={(checked) => setCreateField("mailerEnabled", checked)} label={t("admin.users.create.field.mailer_enabled")} testId="admin.users.create.mailer_enabled" />
                <label className="block sm:col-span-2">
                  <span className="text-sm font-medium">{t("admin.users.create.field.address")}</span>
                  <Input value={createDraft.address} onChange={(e) => setCreateField("address", e.target.value)} testId="admin.users.create.address" />
                </label>
                <label className="block sm:col-span-2">
                  <span className="text-sm font-medium">{t("admin.users.create.field.info")}</span>
                  <textarea className="mt-1 min-h-24 w-full rounded-md border border-border bg-surface px-3 py-2 text-sm text-fg" value={createDraft.info} onChange={(e) => setCreateField("info", e.target.value)} data-testid="admin.users.create.info" />
                </label>
              </div>
            </div>
          </Modal>
    ...
    >
      {listQ.isLoading ? (
        ...
        <ErrorState testId="admin.users.error" title={t("admin.users.load_error")} error={listQ.error} onRetry={() => void listQ.refetch()} showBack={false} detailsExtra={{ page: "admin.users" }} />
      ) : pageData.length === 0 ? (
        <EmptyState testId="admin.users.empty" title={filtersActive ? t("empty.list.no_matches.title") : t("admin.users.empty")} body={filtersActive ? t("empty.list.no_matches.body") : undefined} actionLabel={filtersActive ? t("common.clear_filters") : undefined} onAction={filtersActive ? clearFilters : undefined} />
      ) : (
        <>
          <UsersListMobile users={pageData} basePath={basePath} t={t} />
          {canPaginate ? (
            <Card className="md:hidden">
              <KeysetPagination page={pagination.page} pageCount={pagination.stack.length} canPrev={pagination.canPrev} canNext={canNext} onPrev={pagination.goPrev} onNext={() => pagination.goNext(pageCursor)} onGoToPage={pagination.goToPage} limit={pagination.limit} allowedLimits={pagination.allowedLimits} onLimitChange={pagination.setLimit} testId="admin.users.pagination.mobile" />
            </Card>
          ) : null}
          <UsersListTable users={pageData} basePath={basePath} t={t} na={na} pagination={pagination} canPaginate={canPaginate} canNext={canNext} pageCursor={pageCursor} />
        </>
      )}
    </ListShell>
  );
