...
import { createLocationNetwork, deleteLocationNetwork, fetchLocationNetworks, updateLocationNetwork, type LocationNetwork } from "../../../../lib/api/locationNetworks";
...
export function NetworkDetailPage() {
  ...
  const sortedLns = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      const al = locLabel((a as LegacyAny).location ?? null);
      const bl = locLabel((b as LegacyAny).location ?? null);
  ...
  const addAddressesM = useMutation({
    ...
    onSuccess: async (res) => {
      ...
      pushToast({
        ...
        body: t("admin.cluster.network_detail.add_addresses.toast_body", { count: (res.data as LegacyAny)?.count ?? addAddressesForm.count }),
      });
  ...
  if (!id) {
    return <ErrorState title={t("admin.cluster.network_detail.invalid_title")} message={t("admin.cluster.network_detail.invalid_body")} testId="admin.cluster.network_detail.bad_id" />;
  }
  if (netQ.isLoading) {
    return <LoadingState testId="admin.cluster.network_detail.loading" />;
  }
  if (netQ.isError || !net) {
    return <ErrorState title={t("admin.cluster.network_detail.error.title")} message={t("admin.cluster.network_detail.error.body")} onRetry={() => netQ.refetch()} testId="admin.cluster.network_detail.error" />;
  }
  ...
  return (
    <div className="mt-4 space-y-6" data-testid="admin.cluster.network_detail.page">
      <ObjectHeader
        ...
        badges={
          <>
            ...
            <Badge variant={managed ? "ok" : "neutral"}>{managed ? t("admin.cluster.networks.managed.true") : t("admin.cluster.networks.managed.false")}</Badge>
          </>
        }
        meta={<span>{typeof net.label === "string" && net.label.trim() ? <span className="text-fg">{net.label.trim()}</span> : <span className="text-muted">{t("admin.cluster.network_detail.meta.no_label")}</span>}</span>}
        actions={
          <>
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-6">
        ...
        <StatCard testId="admin.cluster.network_detail.stat.assigned" title={t("admin.cluster.network_detail.stat.assigned")} value={assigned ?? "—"} />
        ...
        <CardBody>
          <dl className="grid gap-x-6 gap-y-3 md:grid-cols-2">
            ...
            <div>
              ...
              <dd className="mt-1 text-fg">{locLabel((net as LegacyAny).primary_location ?? null)}</dd>
            </div>
          </dl>
        </CardBody>
      </Card>
      <Card testId="admin.cluster.network_detail.availability">
        ...
        <CardBody className="space-y-3">
          {lnQ.isLoading ? (
            ...
            <ErrorState title={t("admin.cluster.network_detail.availability.error.title")} message={t("admin.cluster.network_detail.availability.error.body")} onRetry={() => lnQ.refetch()} testId="admin.cluster.network_detail.availability.error" />
          ) : sortedLns.length === 0 ? (
            <EmptyState title={t("admin.cluster.network_detail.availability.empty.title")} message={t("admin.cluster.network_detail.availability.empty.body")} testId="admin.cluster.network_detail.availability.empty" />
          ) : (
                ...
                {sortedLns.map((ln) => {
                  ...
                  return (
                    <tr key={ln.id} data-testid={`admin.cluster.network_detail.ln.${ln.id}`}>
                      <td className="px-3 py-2 text-fg">{locLabel((ln as LegacyAny).location ?? null)}</td>
                      <td className="px-3 py-2">{primary ? <Badge variant="ok">{t("admin.cluster.network_detail.badge.primary")}</Badge> : "—"}</td>
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="inline-flex items-center gap-2">
                          <Button size="sm" variant="secondary" onClick={() => openEdit(ln)} testId={`admin.cluster.network_detail.ln.${ln.id}.edit`}>
                            {t("common.edit")}
                          </Button>
                          <Button size="sm" variant="danger" onClick={() => setDeleteState({ open: true, ln })} testId={`admin.cluster.network_detail.ln.${ln.id}.remove`}>
                            {t("common.remove")}
                          </Button>
                        </div>
                      </td>
        ...
        <div className="space-y-4">
          {editor?.mode === "create" ? (
            <div>
              ...
              <div className="mt-1">
                <Select testId="admin.cluster.network_detail.editor.location" value={form.locationId} onChange={(e) => setForm((p) => ({ ...p, locationId: e.target.value }))} options={locationOptions} />
              </div>
            </div>
          ) : (
            <div>
              <div className="text-xs font-semibold text-muted">{t("common.location")}</div>
              <div className="mt-1 text-fg">{locLabel((editor?.ln as LegacyAny)?.location ?? null)}</div>
            </div>
          )}
          <div>
            ...
            <div className="mt-2">
              <Input testId="admin.cluster.network_detail.editor.priority" value={form.priority} onChange={(e) => setForm((p) => ({ ...p, priority: e.target.value }))} inputMode="numeric" className="font-mono text-xs tabular-nums" />
            </div>
          </div>
          <SwitchRow testId="admin.cluster.network_detail.editor.primary" checked={form.primary} onChange={(v) => setForm((p) => ({ ...p, primary: v }))} label={t("admin.cluster.network_detail.field.primary")} description={t("admin.cluster.network_detail.field.primary_desc")} />
          <SwitchRow testId="admin.cluster.network_detail.editor.autopick" checked={form.autopick} onChange={(v) => setForm((p) => ({ ...p, autopick: v }))} label={t("admin.cluster.network_detail.field.autopick")} description={t("admin.cluster.network_detail.field.autopick_desc")} />
          <SwitchRow testId="admin.cluster.network_detail.editor.userpick" checked={form.userpick} onChange={(v) => setForm((p) => ({ ...p, userpick: v }))} label={t("admin.cluster.network_detail.field.userpick")} description={t("admin.cluster.network_detail.field.userpick_desc")} />
        </div>
      </Modal>
      <ConfirmDialog
        ...
        message={t("admin.cluster.network_detail.remove.body", {
          location: locLabel((deleteState.ln as LegacyAny)?.location ?? null),
        })}
      ...
      <Modal
        ...
        footer={
          <div className="flex items-center justify-end gap-2">
            ...
            <Button variant="primary" loading={addAddressesM.isPending} onClick={() => addAddressesM.mutate()} disabled={!parsePositiveInt(addAddressesForm.count)} testId="admin.cluster.network_detail.add_addresses.save">
      ...
      >
        <div className="space-y-4">
          ...
          <FormField label={t("admin.cluster.network_detail.add_addresses.count")}>
            <Input testId="admin.cluster.network_detail.add_addresses.count" value={addAddressesForm.count} onChange={(e) => setAddAddressesForm((p) => ({ ...p, count: e.target.value }))} inputMode="numeric" />
          </FormField>
          <FormField label={t("admin.cluster.network_detail.add_addresses.user")}>
            <Input testId="admin.cluster.network_detail.add_addresses.user" value={addAddressesForm.user} onChange={(e) => setAddAddressesForm((p) => ({ ...p, user: e.target.value }))} inputMode="numeric" />
          </FormField>
          <FormField label={t("admin.cluster.network_detail.add_addresses.environment")}>
            <Input testId="admin.cluster.network_detail.add_addresses.environment" value={addAddressesForm.environment} onChange={(e) => setAddAddressesForm((p) => ({ ...p, environment: e.target.value }))} inputMode="numeric" />
          </FormField>
        </div>
      </Modal>
    </div>
