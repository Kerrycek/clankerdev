...
export function DnsServersPage() {
  ...
  return (
    <div className="space-y-6" data-testid="admin.cluster.dns_servers.page">
              ...
              <Card key={row.id} testId={`admin.cluster.dns_servers.card.${row.id}`}>
                <div className="min-w-0 p-4">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 border-t border-border pt-3" role="group" aria-label={t('common.actions')}>
                    ...
                    <ActionButton size="sm" variant="danger" className="min-h-11 min-w-0 px-3" title={t('common.delete')} ariaLabel={t('common.delete')} testId={`admin.cluster.dns_servers.card.${row.id}.delete`} onClick={() => { deleteM.reset(); setConfirmDelete(row); }}><Trash2 className="h-4 w-4" aria-hidden />{t('common.delete')}</ActionButton>
                  </div>
                </div>
              </Card>
            ))}
      ...
      >
        {deleteM.isError ? (
          <Alert variant="danger" title={t('common.error')} testId="admin.cluster.dns_servers.delete_error">
