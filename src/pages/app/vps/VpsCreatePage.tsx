...
function locationEnvironmentId(loc: Location | undefined): number | undefined {
  ...
  const raw = loc ? (loc as LegacyAny).environment_id : undefined;
...
export function validateForm(form: FormState, isAdmin: boolean, hiddenAdminTarget?: { userId?: number; nodeId?: number }): string[] {
  const errors: string[] = [];
...
export function VpsCreatePage() {
  ...
  const selectedLocation = useMemo(() => locations.find((loc) => Number(loc.id) === selectedLocationId), [locations, selectedLocationId]);
  ...
  const defaultResourcesQ = useQuery({
    ...
    queryFn: async () => (await fetchDefaultObjectClusterResources({ limit: 50, environmentId: selectedEnvironmentId, className: "Vps" })).data,
    enabled: selectedEnvironmentId !== undefined,
  });
  const nodes = needsAdminPayload ? (nodesQ.data ?? []).filter(isVpsHypervisorNode) : [];
  const hiddenAdminTarget = !isAdminMode && isAdminAccount ? { userId: auth.user?.id, nodeId: nodes[0]?.id } : undefined;
  const templatesByFamily = useMemo(() => {
    const groups = new Map<string, OsTemplate[]>();
    for (const tpl of templatesQ.data ?? []) {
      const family = tpl.os_family as LegacyAny;
  ...
  const validationKeys = useMemo(() => validateForm(form, isAdminMode, hiddenAdminTarget), [form, hiddenAdminTarget, isAdminMode]);
  const canSubmit = validationKeys.length === 0;
  // audit:ignore missing-local-lock -- create returns the VPS object only after submit, so no object lock exists before success tracking.
          ...
          <div className="space-y-4">
            <Card testId="vps.create.target">
              ...
              <CardBody className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <Select value={form.locationId} onChange={(e) => setForm((prev) => ({ ...prev, locationId: e.target.value, nodeId: "" }))} testId="vps.create.location" options={[{ value: "", label: t("common.select") }, ...locations.map((l: Location) => ({ value: String(l.id), label: labelOf(l) }))]} />
                  {selectedLocation?.environment ? <p className="mt-1 text-xs text-muted">{t("vps.create.location_environment", { environment: labelOf(selectedLocation.environment) })}</p> : null}
                </div>
              </CardBody>
            </Card>
            ...
            <Card testId="vps.create.resources">
              ...
              <CardBody className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                  {label(t("vps.create.field.cpu"))}
                  <Input type="number" min="1" max="32" step="1" value={form.cpu} onChange={(e) => update("cpu", e.target.value)} testId="vps.create.cpu" />
                </div>
                <div>
                  {label(t("vps.create.field.memory"))}
                  <Input type="number" min="512" max="131072" step="1" value={form.memory} onChange={(e) => update("memory", e.target.value)} testId="vps.create.memory" />
                </div>
                <div>
                  {label(t("vps.create.field.diskspace"))}
                  <Input type="number" min="1024" max="10485760" step="1" value={form.diskspace} onChange={(e) => update("diskspace", e.target.value)} testId="vps.create.diskspace" />
                </div>
                <div>
                  {label(t("vps.create.field.swap"))}
                  <Input type="number" min="0" max="12288" step="1" value={form.swap} onChange={(e) => update("swap", e.target.value)} testId="vps.create.swap" />
                </div>
                <div>
                  {label(t("vps.create.field.ipv4"))}
                  <Input type="number" min="0" max="64" step="1" value={form.ipv4} onChange={(e) => update("ipv4", e.target.value)} testId="vps.create.ipv4" />
                </div>
                <div>
                  {label(t("vps.create.field.ipv6"))}
                  <Input type="number" min="0" max="64" step="1" value={form.ipv6} onChange={(e) => update("ipv6", e.target.value)} testId="vps.create.ipv6" />
                </div>
                <div>
                  {label(t("vps.create.field.ipv4_private"))}
                  <Input type="number" min="0" max="64" step="1" value={form.ipv4Private} onChange={(e) => update("ipv4Private", e.target.value)} testId="vps.create.ipv4_private" />
                </div>
              </CardBody>
            </Card>
          </div>
