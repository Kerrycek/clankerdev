...
function rootDatasetField(ds: StorageDataset | null | undefined, fallback: any, key: string): unknown {
  if (ds && (ds as LegacyAny)[key] !== undefined) return (ds as LegacyAny)[key];
  return fallback?.[key];
}
export function VpsStoragePage() {
  ...
  const findDatasetM = useMutation({
    ...
    onSuccess: (r) => {
      setFindError(null);
      setFoundDataset(r.data as LegacyAny);
    },
    onError: (e: any) => {
      setFoundDataset(null);
  ...
  const openEdit = (m: VpsMount) => {
    ...
    setEditMount(m);
    setEditOpen(true);
    setMountpoint(String((m as LegacyAny).mountpoint ?? ""));
    setType(((m as LegacyAny).type ?? "nfs") as LegacyAny);
    setMode(((m as LegacyAny).mode ?? "rw") as LegacyAny);
    setOnStartFail(((m as LegacyAny).on_start_fail ?? "ignore") as LegacyAny);
    setEnabled(canonicalBool((m as LegacyAny).enabled, true));
    setMasterEnabled(canonicalBool((m as LegacyAny).master_enabled, true));
    setUseDefaultMap(canonicalBool((m as LegacyAny).use_default_map, true));
  };
  const editDirty = useMemo(() => {
    if (!editMount) return false;
    const mp = mountpoint.trim() !== String((editMount as LegacyAny).mountpoint ?? "");
    const ty = type !== String((editMount as LegacyAny).type ?? "nfs");
    const mo = mode !== String((editMount as LegacyAny).mode ?? "rw");
    const osf = onStartFail !== String((editMount as LegacyAny).on_start_fail ?? "ignore");
    const en = enabled !== canonicalBool((editMount as LegacyAny).enabled, true);
    const me = masterEnabled !== canonicalBool((editMount as LegacyAny).master_enabled, true);
    const dm = useDefaultMap !== canonicalBool((editMount as LegacyAny).use_default_map, true);
  ...
  const submitDelete = async () => {
    ...
    const expected = String((deleteTarget as LegacyAny).mountpoint ?? deleteTarget.id);
  ...
  const rootDatasetId = datasetId((vps as LegacyAny).dataset);
  const rootDatasetLabel = datasetLabel((vps as LegacyAny).dataset);
  ...
  const rootDatasetFallback = (vps as LegacyAny).dataset;
    ...
    <div data-testid="vps.storage.page" className="space-y-4">
      ...
      {rootDatasetId ? (
        <Card testId="vps.storage.root_dataset">
          <CardHeader title={t("vps.storage.root_dataset.title")} subtitle={t("vps.storage.root_dataset.subtitle", { dataset: rootDatasetLabel })} />
          <CardBody>
            {rootDatasetQ.isLoading ? (
              ...
              <div className="mb-3">
                <Alert title={t("vps.storage.root_dataset.load_error")} variant="danger">
                  {String((rootDatasetQ.error as LegacyAny)?.message ?? rootDatasetQ.error)}
                </Alert>
              </div>
            ) : null}
        ...
        <CardBody>
          {mountsQ.isLoading ? (
            ...
            <Alert title={t("vps.storage.mounts.load_error")} variant="danger">
              {String((mountsQ.error as LegacyAny)?.message ?? mountsQ.error)}
            </Alert>
          ) : mounts.length === 0 ? (
            ...
            <>
              ...
              <div className="space-y-3 md:hidden">
                {mounts.map((m) => {
                  const ds = datasetLabel((m as LegacyAny).dataset);
                  const dsId = datasetId((m as LegacyAny).dataset);
                  return (
                    <Card key={m.id} testId={`vps.storage.mounts.card.${m.id}`}>
                      <CardBody>
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="truncate text-base font-semibold">{String((m as LegacyAny).mountpoint ?? "—")}</div>
                          ...
                          <div className="flex shrink-0 items-center gap-2">
                            <Badge variant={canonicalBool((m as LegacyAny).enabled, true) ? "ok" : "warn"}>{canonicalBool((m as LegacyAny).enabled, true) ? t("common.enabled") : t("common.disabled")}</Badge>
                            {canAdmin ? (
                              <Badge variant={canonicalBool((m as LegacyAny).master_enabled, true) ? "neutral" : "warn"}>
                                {t("vps.storage.mounts.master_short")}: {canonicalBool((m as LegacyAny).master_enabled, true) ? t("common.yes") : t("common.no")}
                              </Badge>
                            ) : null}
                          </div>
                        </div>
                        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted">
                          <span>{t("vps.storage.mounts.field.mode", { mode: String((m as LegacyAny).mode ?? "—") })}</span>
                          <span>{t("vps.storage.mounts.field.type", { type: String((m as LegacyAny).type ?? "—") })}</span>
                          <span>{t("vps.storage.mounts.field.on_start_fail", { value: String((m as LegacyAny).on_start_fail ?? "—") })}</span>
                          <span>
                            {t("vps.storage.mounts.field.default_map", {
                              value: canonicalBool((m as LegacyAny).use_default_map, true) ? t("common.yes") : t("common.no"),
                            })}
                          </span>
                          <span>
                            {t("vps.storage.mounts.field.current_state", {
                              value: String((m as LegacyAny).current_state ?? t("common.na")),
                            })}
                          </span>
                          {(m as LegacyAny).expiration_date ? (
                            <span>
                              {t("vps.storage.mounts.field.expiration", {
                                value: formatDateTime((m as LegacyAny).expiration_date),
                              })}
                            </span>
                          ) : null}
                        </div>
                        <div className="mt-3 flex items-center justify-end gap-2">
                          <Button variant="secondary" size="sm" testId={`vps.storage.mounts.card.${m.id}.edit`} onClick={() => openEdit(m)}>
                            {t("common.edit")}
                          </Button>
                          <Button variant="danger" size="sm" testId={`vps.storage.mounts.card.${m.id}.delete`} onClick={() => openDelete(m)}>
                            {t("common.delete")}
                          </Button>
                        </div>
                      </CardBody>
                ...
                <Table testId="vps.storage.mounts.table" minWidth="lg">
                  ...
                  <tbody>
                    {mounts.map((m) => {
                      const ds = datasetLabel((m as LegacyAny).dataset);
                      const dsId = datasetId((m as LegacyAny).dataset);
                      const enabledLabel = canonicalBool((m as LegacyAny).enabled, true) ? t("common.yes") : t("common.no");
                      const masterLabel = canonicalBool((m as LegacyAny).master_enabled, true) ? t("common.yes") : t("common.no");
                      return (
                        <tr key={m.id} data-testid={`vps.storage.mounts.row.${m.id}`} className="border-b border-border/60 last:border-b-0">
                          <td className="px-4 py-3 font-medium">{String((m as LegacyAny).mountpoint ?? "—")}</td>
                          ...
                          <td className="px-4 py-3">{String((m as LegacyAny).mode ?? "—")}</td>
                          <td className="px-4 py-3">{String((m as LegacyAny).type ?? "—")}</td>
                          <td className="px-4 py-3">
                            <Badge variant={canonicalBool((m as LegacyAny).enabled, true) ? "ok" : "warn"}>{enabledLabel}</Badge>
                          </td>
                          {canAdmin ? (
                            <td className="px-4 py-3">
                              <Badge variant={canonicalBool((m as LegacyAny).master_enabled, true) ? "neutral" : "warn"}>{masterLabel}</Badge>
                            </td>
                          ) : null}
                          <td className="px-4 py-3">{String((m as LegacyAny).on_start_fail ?? "—")}</td>
                          <td className="px-4 py-3">{canonicalBool((m as LegacyAny).use_default_map, true) ? t("common.yes") : t("common.no")}</td>
                          <td className="px-4 py-3">{String((m as LegacyAny).current_state ?? "—")}</td>
                          <td className="px-4 py-3 text-xs text-muted">{formatDateTime((m as LegacyAny).expiration_date)}</td>
                          <td className="px-4 py-3 text-xs text-muted">{formatDateTime((m as LegacyAny).created_at)}</td>
                          <td className="px-4 py-3 text-xs text-muted">{formatDateTime((m as LegacyAny).updated_at)}</td>
                          <td className="px-4 py-3 text-right">
                            <div className="inline-flex items-center gap-2">
                              <Button variant="secondary" size="sm" testId={`vps.storage.mounts.row.${m.id}.edit`} onClick={() => openEdit(m)}>
                                {t("common.edit")}
                              </Button>
                              <Button variant="danger" size="sm" testId={`vps.storage.mounts.row.${m.id}.delete`} onClick={() => openDelete(m)}>
                                {t("common.delete")}
                              </Button>
                            </div>
                          </td>
      ...
      <Modal
        ...
        footer={
          <div className="flex items-center justify-end gap-2">
            <Button variant="secondary" testId="vps.storage.mounts.create.cancel" onClick={() => setCreateOpen(false)} disabled={createMountM.isPending}>
              {t("common.cancel")}
            </Button>
            <ActionButton testId="vps.storage.mounts.create.submit" disabled={!canCreate || !gate.allowed} disabledReason={!gate.allowed ? gate.reason : undefined} loading={createMountM.isPending} onClick={() => void submitCreate()}>
              {t("common.create")}
            </ActionButton>
          </div>
        }
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              <Input testId="vps.storage.mounts.create.dataset" value={datasetName} onChange={(e) => setDatasetName(e.target.value)} placeholder={t("vps.storage.dataset_find.placeholder")} autoComplete="off" />
              <Button variant="secondary" size="sm" testId="vps.storage.mounts.create.find_dataset" onClick={() => void findDatasetM.mutate(datasetName)} loading={findDatasetM.isPending}>
                {t("common.find")}
              </Button>
            </div>
            ...
            {foundDataset ? (
              <div className="mt-2 rounded-md border border-border bg-surface-2 p-2 text-sm">
                ...
                <div className="mt-0.5 text-xs text-muted">#{(foundDataset as LegacyAny).id}</div>
              </div>
            ) : null}
          </div>
          <div>
            ...
            <div className="mt-1">
              <Input testId="vps.storage.mounts.create.mountpoint" value={mountpoint} onChange={(e) => setMountpoint(e.target.value)} placeholder="/mnt/data" autoComplete="off" />
            </div>
          </div>
            ...
            <div>
              ...
              <div className="mt-1">
                <Select
                  ...
                  value={type}
                  onChange={(e) => setType(e.target.value as LegacyAny)}
                  options={[
            ...
            <div>
              ...
              <div className="mt-1">
                <Select
                  ...
                  value={mode}
                  onChange={(e) => setMode(e.target.value as LegacyAny)}
                  options={[
          ...
          <div>
            ...
            <div className="mt-1">
              <Select testId="vps.storage.mounts.create.on_start_fail" value={onStartFail} onChange={(e) => setOnStartFail(e.target.value as LegacyAny)} options={onStartFailOptions.map((o) => ({ value: o.value, label: o.label }))} />
            </div>
            <div className="mt-1 text-xs text-muted">{onStartFailOptions.find((o) => o.value === onStartFail)?.desc}</div>
          </div>
          <div className="grid gap-2">
            <label className="flex items-center gap-2 text-sm">
              <input data-testid="vps.storage.mounts.create.enabled" type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="h-4 w-4 rounded border-border bg-surface text-accent focus:ring-2 focus:ring-focus/35 focus:ring-offset-2 focus:ring-offset-bg" />
              <span>{t("vps.storage.create.field.enabled")}</span>
            </label>
            {canAdmin ? (
              <label className="flex items-center gap-2 text-sm">
                <input data-testid="vps.storage.mounts.create.master_enabled" type="checkbox" checked={masterEnabled} onChange={(e) => setMasterEnabled(e.target.checked)} className="h-4 w-4 rounded border-border bg-surface text-accent focus:ring-2 focus:ring-focus/35 focus:ring-offset-2 focus:ring-offset-bg" />
                <span>{t("vps.storage.create.field.master_enabled")}</span>
              </label>
            ) : null}
            <label className="flex items-center gap-2 text-sm">
              <input data-testid="vps.storage.mounts.create.use_default_map" type="checkbox" checked={useDefaultMap} onChange={(e) => setUseDefaultMap(e.target.checked)} className="h-4 w-4 rounded border-border bg-surface text-accent focus:ring-2 focus:ring-focus/35 focus:ring-offset-2 focus:ring-offset-bg" />
              <span>{t("vps.storage.create.field.use_default_map")}</span>
            </label>
          </div>
          <div className="text-xs text-muted">{t("vps.storage.create.basic_hint")}</div>
          <div className="rounded-md border border-border bg-surface-2 p-3 text-xs text-muted">{foundDataset ? t("vps.storage.create.scope", { dataset: datasetLabel(foundDataset), vps: objectLabel }) : t("vps.storage.create.scope_pending", { vps: objectLabel })}</div>
        </div>
      </Modal>
      <Modal
        open={editOpen}
        ...
        footer={
          <div className="flex items-center justify-end gap-2">
            <Button variant="secondary" testId="vps.storage.mounts.edit.cancel" onClick={() => setEditOpen(false)} disabled={updateMountM.isPending}>
              {t("common.cancel")}
            </Button>
            <ActionButton testId="vps.storage.mounts.edit.submit" disabled={!editDirty || !gate.allowed} disabledReason={!gate.allowed ? gate.reason : undefined} loading={updateMountM.isPending} onClick={() => void submitEdit()}>
              {t("common.save")}
            </ActionButton>
          </div>
        }
        ...
        <div className="space-y-4">
          ...
          {editMount ? (
            <div className="rounded-md border border-border bg-surface-2 p-2 text-sm">
              <div className="font-medium">{datasetLabel((editMount as LegacyAny).dataset)}</div>
          ...
          <div>
            ...
            <div className="mt-1">
              <Input testId="vps.storage.mounts.edit.mountpoint" value={mountpoint} onChange={(e) => setMountpoint(e.target.value)} autoComplete="off" />
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
          ...
          <div>
            ...
            <div className="mt-1">
              <Select testId="vps.storage.mounts.edit.on_start_fail" value={onStartFail} onChange={(e) => setOnStartFail(e.target.value as LegacyAny)} options={onStartFailOptions.map((o) => ({ value: o.value, label: o.label }))} />
            </div>
            <div className="mt-1 text-xs text-muted">{onStartFailOptions.find((o) => o.value === onStartFail)?.desc}</div>
          </div>
          <div className="grid gap-2">
            <label className="flex items-center gap-2 text-sm">
              <input data-testid="vps.storage.mounts.edit.enabled" type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="h-4 w-4 rounded border-border bg-surface text-accent focus:ring-2 focus:ring-focus/35 focus:ring-offset-2 focus:ring-offset-bg" />
              <span>{t("vps.storage.edit.field.enabled")}</span>
            </label>
            {canAdmin ? (
              <label className="flex items-center gap-2 text-sm">
                <input data-testid="vps.storage.mounts.edit.master_enabled" type="checkbox" checked={masterEnabled} onChange={(e) => setMasterEnabled(e.target.checked)} className="h-4 w-4 rounded border-border bg-surface text-accent focus:ring-2 focus:ring-focus/35 focus:ring-offset-2 focus:ring-offset-bg" />
                <span>{t("vps.storage.edit.field.master_enabled")}</span>
              </label>
            ) : null}
            <label className="flex items-center gap-2 text-sm">
              <input data-testid="vps.storage.mounts.edit.use_default_map" type="checkbox" checked={useDefaultMap} onChange={(e) => setUseDefaultMap(e.target.checked)} className="h-4 w-4 rounded border-border bg-surface text-accent focus:ring-2 focus:ring-focus/35 focus:ring-offset-2 focus:ring-offset-bg" />
      ...
      <ConfirmDialog
        ...
        confirmationText={deleteTarget ? String((deleteTarget as LegacyAny).mountpoint ?? deleteTarget.id) : undefined}
      ...
      >
        {deleteError ? (
          ...
          <div className="text-sm">
            <div className="font-medium">{String((deleteTarget as LegacyAny).mountpoint ?? "—")}</div>
            <div className="mt-0.5 text-xs text-muted">{datasetLabel((deleteTarget as LegacyAny).dataset)}</div>
          </div>
        ) : null}
      </ConfirmDialog>
    </div>
