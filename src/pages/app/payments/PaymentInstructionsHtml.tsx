...
export function PaymentInstructionsHtml(props: { html: string; className?: string; testId?: string }) {
  const sanitized = useMemo(() => sanitizePaymentInstructionsHtml(props.html), [props.html]);

  return (
    <div
      data-testid={props.testId}
      className={clsx(
        'payment-instructions text-sm leading-relaxed text-strong',
        ...
        '[&_table]:my-3 [&_table]:w-full [&_table]:max-w-full [&_table]:border-collapse [&_table]:overflow-hidden [&_table]:rounded-lg',
        ...
        '[&_img]:h-auto [&_img]:max-w-xs [&_img]:rounded-md [&_img]:bg-white [&_img]:p-1',
