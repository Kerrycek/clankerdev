...
import { acknowledgeMonitoredEvent, fetchMonitoredEvent, fetchMonitoredEventLogs, ignoreMonitoredEvent, type MonitoredEvent, type MonitoredEventLog, type MonitoredEventLogOrder } from "../../lib/api/monitoring";
...
function untilIso(preset: UntilPreset, customLocal: string): { iso?: string; valid: boolean } {
  ...
  const now = Date.now();
  const ms = preset === "1h" ? 60 * 60 * 1000 : preset === "6h" ? 6 * 60 * 60 * 1000 : preset === "1d" ? 24 * 60 * 60 * 1000 : preset === "1w" ? 7 * 24 * 60 * 60 * 1000 : 0;
  if (preset !== "custom") {
...
export function MonitoringEventDetailPage() {
  ...
  const event = eventQ.data as LegacyAny as MonitoredEvent | undefined;
  const stateVal = String((event as LegacyAny)?.state ?? "");
  ...
  const objName = (event as LegacyAny)?.object_name as string | undefined;
  const objId = Number((event as LegacyAny)?.object_id);
  const objLink = objectLink(basePath, objName, Number.isFinite(objId) ? objId : undefined);
  const savedUntilIso = (event as LegacyAny)?.saved_until as string | null | undefined;
  const savedUntilLabel = savedUntilIso ? formatDateTime(savedUntilIso) : stateVal === "acknowledged" || stateVal === "ignored" ? t("monitoring.saved_until.forever") : t("common.na");
  const durationLabel = typeof (event as LegacyAny)?.duration === "number" && Number.isFinite((event as LegacyAny).duration) ? formatDurationSeconds((event as LegacyAny).duration) : t("common.na");
  ...
  const logs = (logsQ.data ?? []) as LegacyAny as MonitoredEventLog[];
  const logCursor = useMemo(() => {
    return logOrder === "latest" ? cursorFromDescendingPage(logs as LegacyAny) : cursorFromAscendingPage(logs as LegacyAny);
  }, [logOrder, logs]);
  ...
  const sparklinePoints: PassFailPoint[] = useMemo(() => {
    ...
    for (const l of logs) {
      const ts = (l as LegacyAny).created_at ? Date.parse(String((l as LegacyAny).created_at)) : NaN;
      if (!Number.isFinite(ts)) continue;
      const ok = Boolean((l as LegacyAny).passed);
      const title = `${formatDateTime(String((l as LegacyAny).created_at))} • ${ok ? t("monitoring.log.passed") : t("monitoring.log.failed")}`;
  ...
  if (!id) {
    return (
      <PageContainer testId="monitoring.event.invalid">
        <ErrorState title={t("monitoring.detail.invalid_id.title")} body={t("monitoring.detail.invalid_id.body")} kindOverride="unexpected" showBack testId="monitoring.event.invalid.error" showDetails={false} />
      </PageContainer>
    );
  }
  return (
    <PageContainer testId="monitoring.event.detail">
      ...
      {eventQ.isLoading ? (
        ...
        <ErrorState testId="monitoring.event.error" title={t("monitoring.detail.load_error.title")} error={eventQ.error} onRetry={() => void eventQ.refetch()} showBack detailsExtra={{ page: "monitoring.event.detail", id, scope: basePath }} />
      ) : !event ? (
        <EmptyState testId="monitoring.event.not_found" title={t("monitoring.detail.not_found.title")} body={t("monitoring.detail.not_found.body")} />
      ) : (
            ...
            <CardHeader
              title={
                <div className="flex items-center gap-2">
                  ...
                  <span className="truncate">{(event as LegacyAny).label ?? (event as LegacyAny).monitor ?? t("common.unknown")}</span>
                </div>
              }
              subtitle={(event as LegacyAny).issue ?? ""}
              actions={<Badge variant={badgeV}>{stateLabel}</Badge>}
            />
            <CardBody>
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <div className="text-xs font-medium text-muted">{t("monitoring.field.monitor_name")}</div>
                  <div className="mt-1 text-sm">{(event as LegacyAny).monitor ?? t("common.na")}</div>
                </div>
                <div>
                  <div className="text-xs font-medium text-muted">{t("monitoring.field.object")}</div>
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-sm">
                    <span>{objName ? `${objName} #${Number.isFinite(objId) ? objId : "?"}` : t("common.na")}</span>
                    {objLink ? (
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm">{formatDateTime((event as LegacyAny).created_at)}</div>
                </div>
                <div>
                  <div className="text-xs font-medium text-muted">{t("common.updated")}</div>
                  <div className="mt-1 text-sm">{formatDateTime((event as LegacyAny).updated_at)}</div>
                </div>
                <div>
            ...
            <CardBody>
              {sparklinePoints.length ? (
                <div className="mb-4">
                  ...
                  <PassFailSparkline points={sparklinePoints} ariaLabel={t("monitoring.logs.sparkline")} statsLabels={{ ok: t("monitoring.log.passed"), failed: t("monitoring.log.failed") }} className="mt-2" testId="monitoring.event.logs.sparkline" />
                </div>
              ) : null}
              {logsQ.isLoading ? (
                <LoadingState testId="monitoring.event.logs.loading" />
              ) : logsQ.isError ? (
                <ErrorState testId="monitoring.event.logs.error" title={t("monitoring.logs.load_error.title")} error={logsQ.error} onRetry={() => void logsQ.refetch()} showBack={false} detailsExtra={{ page: "monitoring.event.logs", id, scope: basePath }} />
              ) : logs.length === 0 ? (
                <EmptyState testId="monitoring.event.logs.empty" title={t("monitoring.logs.empty.title")} body={t("monitoring.logs.empty.body")} />
              ) : (
                <TableCard testId="monitoring.event.logs.table" minWidth="md" footer={<KeysetPagination page={logPagination.page} pageCount={logPagination.stack.length} canPrev={logPagination.canPrev} canNext={canNextLogs} onPrev={logPagination.goPrev} onNext={() => logPagination.goNext(logCursor)} onGoToPage={logPagination.goToPage} limit={logPagination.limit} allowedLimits={logPagination.allowedLimits} onLimitChange={logPagination.setLimit} testId="monitoring.event.logs.pagination" />}>
                  <thead>
                  ...
                  <tbody>
                    {logs.map((l: MonitoredEventLog) => {
                      ...
                      const passed = Boolean((l as LegacyAny).passed);
                      const rowVariant = passed ? "ok" : "danger";
                      const dotVariant = passed ? "ok" : "danger";
                      const created = (l as LegacyAny).created_at ? formatDateTime((l as LegacyAny).created_at) : "";
                      const value = (l as LegacyAny).value;
                      const valueLabel = value == null ? t("common.na") : typeof value === "string" ? value : JSON.stringify(value);
                      return (
                        <TableRowLink
                          key={lid}
                        ...
                        >
                          <td className="px-3 py-2">
                            <StatusDot variant={dotVariant} ariaLabel={passed ? t("monitoring.log.passed") : t("monitoring.log.failed")} />
                          </td>
                          <td className="px-4 py-2 font-mono text-xs">{lid}</td>
                          <td className="px-4 py-2">
                            <Badge variant={passed ? "ok" : "danger"}>{passed ? t("monitoring.log.passed") : t("monitoring.log.failed")}</Badge>
                          </td>
      ...
      <ConfirmDialog open={ackOpen} title={t("monitoring.ack.title")} description={t("monitoring.ack.description")} confirmLabel={t("monitoring.action.acknowledge")} confirmLoading={ackMut.isPending} confirmDisabled={!until.valid} onCancel={() => setAckOpen(false)} onConfirm={() => ackMut.mutate()} testId="monitoring.event.ack">
        <div className="space-y-3">
          <div className="text-sm text-muted">{t("monitoring.until.label")}</div>
          <Select value={untilPreset} onChange={(e) => setUntilPreset(e.target.value as UntilPreset)} testId="monitoring.event.ack.until_preset">
          ...
          {untilPreset === "custom" ? (
            <div>
              <Input type="datetime-local" value={untilCustom} onChange={(e) => setUntilCustom(e.target.value)} testId="monitoring.event.ack.until_custom" />
              {!until.valid ? <div className="mt-1 text-xs text-danger">{t("monitoring.until.invalid")}</div> : null}
            </div>
          ) : null}
          {until.iso ? <div className="text-xs text-muted">{t("monitoring.until.preview", { until: formatDateTime(until.iso) })}</div> : <div className="text-xs text-muted">{t("monitoring.until.preview_forever")}</div>}
        </div>
      </ConfirmDialog>
      <ConfirmDialog open={ignoreOpen} title={t("monitoring.ignore.title")} description={t("monitoring.ignore.description")} confirmLabel={t("monitoring.action.ignore")} confirmLoading={ignoreMut.isPending} confirmDisabled={!until.valid} danger onCancel={() => setIgnoreOpen(false)} onConfirm={() => ignoreMut.mutate()} testId="monitoring.event.ignore">
        <div className="space-y-3">
          <div className="text-sm text-muted">{t("monitoring.until.label")}</div>
          <Select value={untilPreset} onChange={(e) => setUntilPreset(e.target.value as UntilPreset)} testId="monitoring.event.ignore.until_preset">
            <option value="1h">{t("monitoring.until.1h")}</option>
            <option value="6h">{t("monitoring.until.6h")}</option>
            <option value="1d">{t("monitoring.until.1d")}</option>
            <option value="1w">{t("monitoring.until.1w")}</option>
          ...
          {untilPreset === "custom" ? (
            <div>
              <Input type="datetime-local" value={untilCustom} onChange={(e) => setUntilCustom(e.target.value)} testId="monitoring.event.ignore.until_custom" />
              {!until.valid ? <div className="mt-1 text-xs text-danger">{t("monitoring.until.invalid")}</div> : null}
            </div>
          ) : null}
          {until.iso ? <div className="text-xs text-muted">{t("monitoring.until.preview", { until: formatDateTime(until.iso) })}</div> : <div className="text-xs text-muted">{t("monitoring.until.preview_forever")}</div>}
