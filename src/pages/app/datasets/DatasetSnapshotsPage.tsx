...
function snapshotDownloadUrl(dl: SnapshotDownload | null): string | undefined {
  if (!dl) return undefined;
  const raw = (dl as any).download_url ?? dl.url;
...
export function DatasetSnapshotsPage() {
  ...
  const createdDownloadHref = snapshotDownloadUrl(createdDownload);
        ...
        <div className="space-y-4" data-testid="dataset.snapshots.download.created">
          ...
          {createdDownload ? (
            <div className="rounded-md border border-border bg-surface-2 p-3 text-sm">
              <div className="font-medium">{t('dataset.downloads.item_title', { id: createdDownload.id })}</div>
              <div className="mt-1 text-xs text-muted">
                {createdDownload.ready === false
                  ? t('dataset.downloads.state.pending')
                  : createdDownloadHref
                    ? createdDownloadHref
                    : t('dataset.downloads.state.unknown')}
              </div>
            </div>
          ) : null}
        </div>
