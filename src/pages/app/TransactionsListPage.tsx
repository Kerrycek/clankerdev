...
import { buildTransactionItemFilterHref, buildTransactionItemRow, canonicalTransactionItemKey, inferDoneToken, inferSuccessToken, parseDone, parseSuccess, type DoneValue } from "./transactions/transactionItemSemantics";
...
export function TransactionsListPage() {
  ...
  const userSuggestQuery = useQuery({
    ...
    enabled: mode === "admin" && debouncedSmartNeedle.length >= 2 && debouncedSmartNeedle !== "?" && !debouncedSmartNeedle.includes(":") && !debouncedSmartNeedle.includes(" ") && parseNumericToken(debouncedSmartNeedle) === null,
  ...
  return (
    ...
    >
      {primaryLoading ? (
        ...
        <>
          <EmptyState testId="transactions.items.empty" title={emptyTitle} body={emptyBody} actionLabel={filtersActive ? t("common.clear_filters") : undefined} onAction={filtersActive ? clearFilters : undefined} />
          <Card className="mt-4">
            <KeysetPagination page={pagination.page} pageCount={pagination.stack.length} canPrev={pagination.canPrev} canNext={canNext} onPrev={pagination.goPrev} onNext={() => pagination.goNext(pageCursor)} onGoToPage={pagination.goToPage} limit={pagination.limit} allowedLimits={pagination.allowedLimits} onLimitChange={pagination.setLimit} testId="transactions.items.pagination" />
          </Card>
        </>
      ) : (
        <TransactionItemsTable rows={rows} basePath={basePath} t={t} mode={mode} pagination={pagination} canNext={canNext} pageCursor={pageCursor} filterHrefArgs={filterHrefArgs} />
      )}
    </ListShell>
  );
}
