...
test.describe('Dataset downloads keyset pagination', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const makeDl = (id: number) => ({
      id,
      dataset: 10,
      snapshot: { id: id + 1000, name: `snap-${id + 1000}`, label: `Snapshot ${id + 1000}` },
      format: id % 2 === 0 ? 'archive' : 'incremental_stream',
      size: 1024 * 1024,
      sha256: '00'.repeat(32),
      url: `/download/${id}`,
      ready: true,
      expiration_date: '2026-01-27T00:00:00.000Z',
    });
    ...
    await installHaveApiMock(page, {
      ...
      handlers: {
        ...
        'GET snapshot_downloads': ({ searchParams }) => {
          ...
          if (ds !== '10') return { snapshot_downloads: [], _meta: { total_count: 0 } };
          if (q) {
            return {
              snapshot_downloads: page1.filter((dl) => String(dl.id) === q || String(dl.file_name).includes(q)),
              _meta: { total_count: 1 },
            };
          }
          return { snapshot_downloads: fromId ? page2 : page1, _meta: { total_count: 100 } };
        },
      },
    });
  });
